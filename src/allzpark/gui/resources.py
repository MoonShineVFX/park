...
class Resources:
    ...
    icons_ext = ".png", ".svg"
    qrc_updated = 1660120478  # !!<qrc-update-time>!! don't touch
...
def validate_qrc():
    ...
    allzpark_gui = here
    allzpark_rc_ = allzpark_gui / "allzpark_rc.py"

    if not allzpark_rc_.is_file():
        log.warning("q-resource file not exists.")
        return False

    try:
        with open(allzpark_rc_, "r") as r:
...
def compile_qrc(timestamp):
    ...
    for fn in Res.fonts:
        q_res.append(f"\n    <file>fonts/{fn}</file>")
    # re-write resources/allzpark-rc.qrc
    with open("allzpark-rc.qrc", "w") as file:
        file.write(f"""
...
""")
    ...
    subprocess.check_output(
        [str(rcc_exec), "allzpark-rc.qrc", "-o", "allzpark_rc.py"], cwd=str(resources)
    )
    shutil.move("allzpark_rc.py", allzpark_gui / "allzpark_rc.py")

    # change to use vendorized Qt5.py
    os.chdir(allzpark_gui)
    with open("allzpark_rc.py", "r") as r:
        s = r.read().replace(
            (
    ...
    with open("allzpark_rc.py", "w") as w:
        w.write(s)
